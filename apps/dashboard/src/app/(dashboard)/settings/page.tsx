        ...
        <div className="px-6 py-5">
          ...
          <dl className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <dd className="font-mono text-gray-700">{workspace.slug}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-gray-500">Created</dt>
              <dd className="text-gray-700">{formatDate(workspace.created_at)}</dd>
            </div>
          </dl>
        </div>
      </section>
          ...
          <dl className="space-y-3 text-sm">
            ...
            {workspace.billing_email && (
              <div className="flex justify-between">
                ...
                <dd className="text-gray-700">{workspace.billing_email}</dd>
              </div>
            )}
            <div className="flex justify-between">
              <dt className="text-gray-500">Message limit / month</dt>
              <dd className="text-gray-700">{workspace.monthly_message_limit.toLocaleString()}</dd>
            </div>
            {workspace.trial_ends_at && (
              <div className="flex justify-between">
                <dt className="text-gray-500">Trial ends</dt>
                <dd className="text-gray-700">{formatDate(workspace.trial_ends_at)}</dd>
              </div>
            )}
          </dl>
          ...
          {isAdmin && (
            <div className="mt-5 pt-5 border-t flex gap-3">
              <a
                ...
                className="px-4 py-2 border text-sm text-gray-700 rounded-lg hover:bg-gray-50 transition-colors"
              >
                Manage billing →
              </a>
              <a
          ...
          {members?.map((m) => (
            <div key={m.id} className="flex items-center gap-4 px-6 py-3.5">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-gray-900 capitalize">{m.role}</p>
