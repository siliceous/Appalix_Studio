...
import { Wallet, Plus, ArrowDownLeft, ArrowUpRight, RefreshCw, CheckCircle, AlertTriangle, Clock } from 'lucide-react'
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white dark:bg-[#1e2535] rounded-2xl border border-gray-200 dark:border-white/10 p-6">
        ...
        >
          ...
          {redirecting ? 'Redirecting to Stripe…' : `Add ${currency} ${(selected / 100).toFixed(2)}`}
        </button>
      </div>
              ...
              <div key={tx.id} className="flex items-center gap-4 px-6 py-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    {new Date(tx.created_at).toLocaleString('en-AU', { dateStyle: 'medium', timeStyle: 'short' })}
                  </p>
                </div>
                <div className="text-right shrink-0">
                  <p className={`text-sm font-semibold tabular-nums ${
                    ...
                    {tx.amount >= 0 ? '+' : ''}{tx.currency} {formatAud(tx.amount)}
                  </p>
                  <p className="text-[10px] text-gray-400 mt-0.5">
                    Balance: {tx.currency} {Number(tx.balance_after).toFixed(2)}
                  </p>
                </div>
              </div>
            ))}
