...
export function MetaEmbeddedSignup({ platform, name, botId, appId, configId }: Props) {
  ...
  function handleConnect() {
    ...
    if (!window.FB || typeof window.FB.login !== 'function') {
      setLoading(false)
      setError('Facebook SDK not ready. Hard-refresh the page (Cmd+Shift+R) and try again.')
      return
    }
      ...
      window.FB.login(
        (res: { authResponse?: { accessToken: string }; status: string; grantedScopes?: string }) => {
          ...
          if (platform === 'facebook_messenger') {
            // Fetch available pages so user can pick
            fetch('/api/oauth/meta/list-pages', {
              ...
              .then(data => {
                ...
                if (data.pages.length === 1) {
                  // Only one page — connect immediately
                  createIntegration(data.longToken ?? token, data.pages[0])
                } else {
  ...
  function createIntegration(token: string, selectedPage: Page | undefined) {
    setLoading(true)
    setPages([])
    fetch('/api/oauth/meta/exchange', {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({ token, platform, name, botId, selectedPage }),
    })
  ...
  const enabled = sdkReady && !loading && pages.length === 0

  return (
    <div>
      {/* Page picker — shown after login when multiple pages exist */}
      {pages.length > 0 && (
        <div className="mb-4">
          ...
          <div className="space-y-2">
            {pages.map(page => (
              <button
                key={page.id}
                type="button"
                onClick={() => createIntegration(longToken, page)}
