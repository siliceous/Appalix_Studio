...
export const metadata: Metadata = { title: 'SMS Conversation' }
...
export default async function SmsDetailPage({
  ...
  const [convRes, msgsRes, listRes, autoSettings, membersRes, profilesRes] = await Promise.all([
    ...
    supabase
      ...
      .order('created_at', { ascending: true }),
    // List scoped to SMS only for prev/next navigation
    supabase
      .from('conversations')
