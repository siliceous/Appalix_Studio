...
import {
  ...
  UserPlus, Ticket, ClipboardList, Sparkles,
} from 'lucide-react'
...
export function SubmissionPanelClient({
  ...
  const [localAssign,   setLocalAssign]   = React.useState<string>(current.assigned_to ?? '')
  const [saving,        setSaving]        = React.useState<'priority' | 'assign' | null>(null)
  const [actionLoading, setActionLoading] = React.useState<'deal' | 'ticket' | 'analyse' | 'delete' | null>(null)
  ...
  async function handleCreateDeal() {
    ...
    const res = await formSubmissionCreateLead(current)
  ...
  async function handleAnalyse() {
    ...
    setActionLoading(null)
    if (res.error) {
      setAnalyseMsg(`Error: ${res.error}`)
    } else {
      setAnalyseMsg(`Analysed ${res.analyzed} submission${res.analyzed !== 1 ? 's' : ''}`)
      router.refresh()
    }
  }
    ...
    <div className="flex h-full overflow-hidden w-full">
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {form && <p className="text-[10px] text-gray-300 dark:text-gray-600 truncate mt-0.5">{form.name}</p>}
        ...
        <div className="shrink-0 bg-white dark:bg-[#232323] border-b dark:border-white/8 px-4 py-2.5">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="min-w-0 mr-2">
              ...
              {currentForm && (
                <p className="text-[10px] text-gray-400 leading-tight">via {currentForm.name}</p>
              )}
            </div>
          ...
          {analyseMsg && (
            <p className="mt-1.5 text-xs text-purple-600 dark:text-purple-400">{analyseMsg}</p>
          )}
        </div>
              ...
              {Object.entries(current.fields).map(([key, value]) => (
                <div key={key} className="bg-white dark:bg-[#2a2a2a] rounded-xl border border-gray-100 dark:border-white/8 px-4 py-3">
                  ...
                  <p className="text-sm text-gray-800 dark:text-gray-100 leading-relaxed whitespace-pre-wrap break-words">
                    {value || <span className="text-gray-300 dark:text-gray-600 italic">empty</span>}
                  </p>
                </div>
              ))}
            </div>
      ...
      <div className="w-[320px] shrink-0 overflow-y-auto bg-white dark:bg-[#232323] border-l dark:border-white/8">
        <div className="p-4 space-y-5">
            ...
            <div className="space-y-2.5">
              ...
              {current.action_type && (
                <DetailRow label="Status" value={
                  ...
                  current.action_type === 'ignored' ? 'Ignored' :
                  current.action_type
                } />
              )}
          ...
          {/* User details */}
          {(contactName || contactEmail || contactPhone || current.ai_entities?.product_interest) && (
            <div>
