...
import {
  Phone, MessageSquare, Download, Search, X, Pencil, Trash2, Loader2,
...
// ── Component ─────────────────────────────────────────────────────────────────
export function CallsPanelClient({
      ...
      nodes.push(
        <div key={msg.id} className={`flex flex-col gap-0.5 ${isBot ? 'items-end' : 'items-start'}`}>
          <span className="text-[10px] text-gray-400 px-1">
            {isBot
              ? 'Agent'
              : (current.ai_entities?.name ?? (current as any).platform_thread_id ?? 'Caller')
            }
          </span>
    ...
    <div className="flex h-full w-full gap-3 p-3 bg-[#f5f4f1] dark:bg-[#1c1c1c]">
      ...
      <div className="w-[240px] shrink-0 flex flex-col bg-white dark:bg-[#181818] rounded-2xl shadow-xl border border-gray-200/60 dark:border-white/8 overflow-hidden">
        ...
        <div className="px-3 py-2.5 bg-[#141c2b] border-b border-white/10 shrink-0 space-y-2">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-white">Phone Calls</h2>
            <Link href="/dashboard/calls" className="text-sm text-white hover:opacity-70 transition-opacity">← Back</Link>
          </div>
      ...
      <div className="flex-1 flex flex-col bg-white dark:bg-[#232323] rounded-2xl shadow-xl border border-gray-200/60 dark:border-white/8 overflow-hidden">
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="shrink-0 bg-[#141c2b] border-b border-white/10 px-4 py-2.5 flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 min-w-0 flex-1">
              ...
              <div className="min-w-0">
                ...
                {(current as any).platform_thread_id && (
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              <div className="flex items-center border border-white/20 rounded-lg overflow-hidden ml-0.5">
                <button onClick={() => prevId && router.push(`/dashboard/calls/${prevId}`)} disabled={!prevId} title="Previous call" className="p-1.5 text-white hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed transition-colors border-r border-white/20">
                  <ChevronUp className="w-3.5 h-3.5" />
                </button>
                <button onClick={() => nextId && router.push(`/dashboard/calls/${nextId}`)} disabled={!nextId} title="Next call" className="p-1.5 text-white hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
          ...
          {/* Read-only footer */}
          <div className="shrink-0 border-t dark:border-white/8 bg-white dark:bg-[#232323] px-4 py-2.5 text-center text-xs text-gray-400">
            Voice call transcript — read only
          </div>
        </div>
      </div>
      ...
      <div className="w-[320px] shrink-0 flex flex-col bg-white dark:bg-[#232323] rounded-2xl shadow-xl border border-gray-200/60 dark:border-white/8 overflow-hidden">
        ...
        <div className="p-4 space-y-5 overflow-y-auto flex-1">
          ...
          <div className="flex flex-wrap gap-1.5">
            <span className="inline-flex items-center text-xs px-2.5 py-1 rounded-full font-medium bg-green-50 text-green-700 dark:bg-green-500/10 dark:text-green-400 border border-green-200 dark:border-green-500/20">
              Voice Call
            </span>
            {localPriority && (
          ...
          {(current.ai_entities?.email || current.ai_entities?.phone || current.ai_entities?.name) && (
            <div>
              <p className="text-[10px] text-gray-400 uppercase tracking-wider mb-3">Caller Details</p>
              <div className="space-y-2.5">
      ...
      {showEmailModal && customerEmail && (
        <EmailComposeModal
          ...
          subject={`Re: ${current.title ?? 'Your call'}`}
      ...
      {showDealPicker && (
        <PipelinePickerModal
          prefill={{
            title:          current.ai_entities?.product_interest ?? current.title ?? `Phone call`,
