...
export default async function CallsPage({
  ...
  let query: any = supabase
    ...
    .eq('platform', 'voice')         // ← locked to voice/call platform
    .is('deleted_at', null)
  ...
  const [activeCountRes, completedCountRes, archivedCountRes] = await Promise.all([
    supabase.from('conversations').select('id', { count: 'exact', head: true }).eq('workspace_id', workspaceId).eq('platform', 'voice').eq('status', 'active').is('deleted_at', null),
    supabase.from('conversations').select('id', { count: 'exact', head: true }).eq('workspace_id', workspaceId).eq('platform', 'voice').eq('status', 'completed').is('deleted_at', null),
    supabase.from('conversations').select('id', { count: 'exact', head: true }).eq('workspace_id', workspaceId).eq('platform', 'voice').eq('status', 'archived').is('deleted_at', null),
  ])
  const statusCounts = {
