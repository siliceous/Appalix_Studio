...
import { FolderOpen, Search, Trophy, Building2, User, Calendar, DollarSign } from 'lucide-react'
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            All won deals — {projects.length} project{projects.length !== 1 ? 's' : ''}
            {totalValue > 0 && (
              <span className="ml-2 text-[#1f6157] dark:text-[#61c2ad] font-medium">
                · {formatCurrency(totalValue, currency)} total value
              </span>
            )}
          </p>
        </div>
        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-[#61c2ad]/10 dark:bg-[#61c2ad]/15 border border-[#61c2ad]/30 rounded-lg">
          <Trophy className="w-3.5 h-3.5 text-[#1f6157] dark:text-[#61c2ad]" />
          <span className="text-xs font-semibold text-[#1f6157] dark:text-[#61c2ad]">Won deals</span>
        </div>
      </div>
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm text-gray-400">
              {search ? 'No projects match your search.' : 'No won deals yet. Close your first deal to see it here.'}
            </p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b dark:border-white/8 bg-gray-50 dark:bg-white/[0.03]">
                ...
                <th className="text-right px-5 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">Won</th>
              </tr>
            </thead>
